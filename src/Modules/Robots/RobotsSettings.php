 ...
 *
 * Holds the robots.txt mode, the optional custom directives block and the
 * per-crawler AI policy map. The custom block can be large, so autoload is
 * disabled. The Sitemap line is owned by the Sitemaps module, never here.
 */
final class RobotsSettings {
	...
	public function all(): array {
		...
		$merged['crawlers'] = CrawlerPolicy::sanitizeMap( $merged['crawlers'] ?? [] );
	...
	private function sanitize( string $key, mixed $value ): mixed {
		switch ( $key ) {
			...
			case 'crawlers':
				return CrawlerPolicy::sanitizeMap( $value );
		}

...
/**
 * Crawl Signals module: virtual robots.txt and per-crawler AI policy.
 *
 * Optional and default off. When disabled nothing is built and no hook is
 * registered. robots.txt is always virtual: the module filters the
 * WordPress output and never writes a physical file. The Sitemap line is
 * left to the Sitemaps module.
 */
class RobotsModule implements ModuleInterface {
	...
	/**
	 * Llms.txt router, built only when llms.txt is enabled.
	 *
	 * @var LlmsRouter|null
	 */
	...
	/**
	 * Llms.txt file writer, built only when llms.txt is enabled.
	 *
	...
	public function isEnabled(): bool {
		...
		$this->enabledCache = array_key_exists( 'robots', $map ) ? (bool) $map['robots'] : in_array( 'robots', $map, true );

		return $this->enabledCache;
	}

	/**
	 * Register services. Nothing heavy is built here.
	 */
	...
	public function boot(): void {
		...
		if ( (bool) $this->llmsSettings->get( 'enabled', false ) ) {
			...
			$this->llmsRouter = new LlmsRouter( new LlmsGenerator(), $this->llmsSettings );
	 ...
	 *
	 * Reserved for the llms.txt route that joins this module; harmless when
	 * no rewrite rule is registered.
	 */

...
| Feature / Functionality | Rank Math Free | Yoast Free | Rank Math PRO | Yoast Premium | RankKernel Status | Notes & RankKernel Edge |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| Per-post SEO title editing UI | yes | yes | yes | yes | PARTIAL | **RankKernel Edge:** free, and built on issue #27 as a Classic Editor meta box plus a Gutenberg sidebar. Unit and contract tested; the sidebar renders it, while save/reload persistence and the Classic Editor surface remain unverified. |
| Per-post meta description editing UI | yes | yes | yes | yes | PARTIAL | **RankKernel Edge:** free, same surface as the title field, with template fallback and a per field reset. Built on issue #27; the sidebar renders it, while save/reload persistence and the Classic Editor surface remain unverified. |
| Per-post robots editing UI | yes | yes | yes | yes | PARTIAL | **RankKernel Edge:** free. Issue #27 adds the directives UI and routes output through the single core wp_robots tag with most restrictive wins. The sidebar renders the controls; frontend robots output after save remains unverified. |
| Per-post canonical override | yes | yes | yes | yes | PARTIAL | **RankKernel Edge:** free. Issue #27 adds the override field and unhooks core rel_canonical so exactly one canonical is emitted. The sidebar renders the field; the frontend canonical result after save remains unverified. |
| Per-post Open Graph and Twitter editing UI | yes | yes | yes | yes | PARTIAL | **RankKernel Edge:** free, including media library pick and remove for both images and a card type selector. Built on issue #27; the sidebar renders the fields, while save/reload persistence and the media select/replace/remove cycle remain unverified. |
| Snippet or SERP preview | yes | yes | yes | yes | PARTIAL | **RankKernel Edge:** free. Issue #27 adds a live preview with desktop and mobile frames and pixel and character budgets. The frames are browser verified; modal to sidebar state propagation and save/reload persistence remain unverified. |
| Social network previews | yes | no | yes | yes | PARTIAL | **RankKernel Edge:** free, where Yoast paywalls it. Issue #27 adds a live social unfurl card that falls back to the General values. The Social tab fields render; the live unfurl card and the social image select/replace/remove cycle remain unverified. |

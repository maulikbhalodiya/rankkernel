...
final class SitemapCacheTest extends TestCase {
    ...
    public function test_default_on_cache_hit_skips_builder(): void {
        ...
        $calls = 0;
        $xml1  = $cache->get('post', 1, static function () use (&$calls): string {
            ++$calls;
        ...
        $calls = 0;
        $xml2  = $cache->get('post', 1, static function () use (&$calls): string {
            ++$calls;
    ...
    public function test_validator_mismatch_triggers_rebuild(): void {
        ...
        $calls = 0;
        $xmlHit = $cache->get('post', 1, static function () use (&$calls): string {
            ++$calls;
        ...
        $calls = 0;
        $xmlRebuilt = $cache->get('post', 1, static function () use (&$calls): string {
            ++$calls;
            return '<new>rebuild</new>';
        });
    ...
    public function test_invalidation_queue_flushes_on_shutdown_once(): void {
        ...
        Functions\when('add_action')->alias(
            static function (string $hook, mixed $callback, int $prio = 10, int $accepted = 1) use (&$shutdownCallback): bool {
                if ('shutdown' === $hook) {

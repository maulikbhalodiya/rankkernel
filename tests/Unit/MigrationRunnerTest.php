...
final class MigrationRunnerTest extends TestCase {
    ...
    public function test_no_pending_ledger_current_single_get_option(): void {
        ...
        $called = false;
        $runner->register('0.1.0', static function () use ( &$called ): void {
            $called = true;
        });
    ...
    public function test_two_pending_run_in_ascending_version_order(): void {
        ...
        $runner->register('0.2.0', static function () use ( &$order ): void {
            $order[] = '0.2.0';
        });
        $runner->register('0.1.0', static function () use ( &$order ): void {
            $order[] = '0.1.0';
        });
    ...
    public function test_migration_failure_fires_action_and_does_not_advance_ledger(): void {
        ...
        $runner = new MigrationRunner();
        $runner->register('0.1.0', static function () use ( $throwable ): void {
            throw $throwable;
        });

        $secondCalled = false;
        $runner->register('0.2.0', static function () use ( &$secondCalled ): void {
            $secondCalled = true;
        });
        ...
        Functions\when('wp_trigger_error')->alias(
            static function ( string $function, string $message, int $type ) use ( &$triggered ): void {
                $triggered = true;
    ...
    public function test_idempotent_double_maybe_run(): void {
        ...
        $runner = new MigrationRunner();
        $runner->register('0.1.0', static function () use ( &$count ): void {
            ++$count;
        });
